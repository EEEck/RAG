...
### **Our Hybrid Schema**
1. **`structure_nodes` (Relational)**:
   - **Purpose**: Defines the rigid hierarchy of a book (Book -> Unit -> Chapter -> Lesson).
   - **Why Relational?**: Hierarchies require strict parent-child relationships (`parent_id`) to ensure integrity. You can't have a Lesson without a Unit.

2. **`content_atoms` (Vector + NoSQL)**:
   - **Purpose**: Stores the actual content (text snippets, image descriptions) for search.
   - **Why NoSQL/Vector?**: Content is messy. A "Vocabulary List" has different metadata than a "Grammar Rule" or an "Image Description".
   - **Implementation**: Uses `pgvector` for the embedding column and `JSONB` for metadata.

### **Atom Types & Pydantic Schemas**
To prevent "NoSQL Chaos" (messy metadata), we enforce schemas at the application level using **Pydantic**:

* **`LanguageMetadata`**: For language books. Enforces fields like `vocabulary_type`, `grammar_topic`.
* **`STEMMetadata`**: For science books. Enforces `formula_latex`, `experiment_type`.
* **`HistoryMetadata`**: For history books. Enforces `era`, `event_date`.

We strictly categorize content atoms into types:
* **`text`**: Standard text extracted from the page.
* **`image_asset`**: A reference to a raw image file cropped from the PDF.
* **`image_desc`**: A semantic description of that image (generated by AI), linked back to the `image_asset`.
...
* **New Way (Multimodal):** We use a **Vision Enrichment Engine**.
  1. We crop the image from the PDF.
  2. We send it to a Vision Language Model (GPT-4o).
  3. The model generates a description: *"An exercise box containing the word 'Apple' with an illustration."*
  4. We embed this **description** (text) so it becomes searchable. The original image is linked but not directly embedded.
