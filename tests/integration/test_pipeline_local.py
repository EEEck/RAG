...
from tests.utils.sqlite_repo import SQLiteStructureNodeRepository
...
def test_pipeline_local_integration(sqlite_db_path, storage_context):
    ...
    repo = SQLiteStructureNodeRepository(db_path=sqlite_db_path)

    try:
        repo.ensure_schema()
        repo.insert_structure_nodes(nodes)

        # Verify Insertion
        # We access the connection directly just for verification in test
        ...
        pass # Repo doesn't hold open connection in this simple impl
